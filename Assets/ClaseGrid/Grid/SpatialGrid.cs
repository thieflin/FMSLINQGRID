...
public class SpatialGrid : MonoBehaviour
{
    ...
    public IEnumerable<GridEntity> Query(Vector3 aabbFrom, Vector3 aabbTo, Func<Vector3, bool> filterByPosition)
    {
        ...
        fromCoord = Tuple.Create(Utilities.Clampi(fromCoord.Item1, 0, width), Utilities.Clampi(fromCoord.Item2, 0, height));
        toCoord = Tuple.Create(Utilities.Clampi(toCoord.Item1, 0, width), Utilities.Clampi(toCoord.Item2, 0, height));
    ...
    private void OnDrawGizmos()
    {
        ...
        else
        {
            ...
            //            ent.myPos = GetPositionInGrid(ent.transform.position);

            //            Debug.Log(ent.myPos.Item1 + " + " + ent.myPos.Item2 + " + " + ent.gameObject.name);


            //            if (!ent.entityInSameCell.Contains(n) && ent.myPos.Item1 == n.myPos.Item1 && ent.myPos.Item2 == n.myPos.Item2)
            //            {
            //                ent.entityInSameCell.Add(n);
            //            }
            //            else
            //            {
            //                ent.entityInSameCell.Remove(n);
            //            }
            //            connections++;
            //        }
            //        if (showLogs)
